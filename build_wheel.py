...
def main() -> None:
    ...
    if version_config is None:
        logger.error(
            "Missing the version key in [tool.setuptools.dynamic] section"
        )
        sys.exit(1)
    ...
    if not match:
        logger.error(
            f"No <major>.<minor>(.<patch>)? version found in {version_path}"
        )
        sys.exit(1)
    ...
    for commit_sha in output.strip().splitlines():
        output = run(
            ["git", "show", f"{commit_sha}:{version_path}"], cwd=root_dir
        )
